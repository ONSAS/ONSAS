...
%
function pelem = womIL(Uprvect1, Uprvect2, Udotdottp1k1, Udotdottp1k2, D, tnp1, dt, Kelem)
...
function [pnp1 dpnp1] = computep(ddXelem, D, tnp1, dt, U, pn, dpn) % at node i= 1,2
  ...
  kp = 4 * omegaf^2;
  [t, pode] = ode45(@(t, p) vanDerPol(t, p, cp, kp, ddXelem, D, Ax), t, [pn, dpn]);
  pnp1 = pode(end, 1);
  dpnp1 = pode(end, 2);
end

function dp = vanDerPol(t, pn, cp, kp, ddXnp1, D, A)
  pdot = pn(2);
