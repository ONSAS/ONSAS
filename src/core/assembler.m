...
function [fsCell, stressMat, tangMatsCell, localInternalForces, strain_vec, acum_plas_strain_vec] = assembler(Conec, elements, Nodes, materials, KS, Ut, Udott, Udotdott, analysisSettings, outputBooleans, nodalDispDamping, timeVar, previousStateCell, rotMatCell)
  ...
  fsBool   = outputBooleans(1);
  stressBool = outputBooleans(2);
  tangBool = outputBooleans(3);
  matFintBool = outputBooleans(4);
  ...
  for elem = 1:nElems
    ...
    elseif strcmp(elemType, 'frame')
      ...
      elseif strcmp(modelName, 'elastic-rotEngStr')
        ...
        [fs, ks, stress, rotData, fintLocCoord] = frameInternalForce(elemNodesxyzRefCoords, ...
                                                                     elemCrossSecParams, ...
                                                                     [1 modelParams], ...
                                                                     elemDisps);
        % rotMat = rotMatCell(nodeselem) ;
        % [~, ~, ~, ~, ~] = frameInternalForce_jv(elemNodesxyzRefCoords, ...
        %                                                              elemCrossSecParams, ...
        %                                                              [1 modelParams], ...
        %                                                              elemDisps, rotMat);
        ...
        if dynamicProblemBool
          [fs, ks] = frameInertialForce(elemNodesxyzRefCoords, elemCrossSecParams, ...
      ...
      if aeroBool && fsBool
        [FaeroElem, MataeroEelem] = frameFluidForce(elemNodesxyzRefCoords,        ...
                                                    elemCrossSecParams,        ...
                                                    elemDisps,        ...
                                                    dotdispsElem,        ...
                                                    dotdotdispsElem,        ...
