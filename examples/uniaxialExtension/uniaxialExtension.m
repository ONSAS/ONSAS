...
% md
% md### Numeric and Analytic solutions computation
% mdThe numerical displacements of the 7th node are computed, as well as the load factors applied
controlDisps                 = matUs(6 * 6 + 1, :);
controlDispsValsCase1        = controlDisps;
loadFactorNumericalValsCase1 = loadFactorsMat;
% mdThe function of the analytic loadfactor for any displacement of node 7 is used
...
base_msh = '';
if strcmp(getenv('TESTS_RUN'), 'yes') && isfolder('examples') % hidden
  base_msh = ['.' filesep 'examples' filesep 'uniaxialExtension' filesep]; % hidden
end % hidden
%
