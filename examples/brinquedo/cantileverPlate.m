...
% otherParams.problemName  = 'cantileverPlate_Linear';
...
controlDisps_z = -matUs(controldofs_z,:)
controlDisps_x = matUs(controldofs_x,:)
loadFactors  =  loadFactorsMat(:, 2)

Iy = Ly*tz^3/12 ;

f_ana = qz*loadFactors*Ly*Lx^4 / (8*E*Iy) 
...
figure;
grid on;
plot(controlDisps_z, loadFactors, 'b-x', 'linewidth', lw, 'markersize', ms);
hold on;
plot(controlDisps_x, loadFactors, 'g-o', 'linewidth', lw, 'markersize', ms);
