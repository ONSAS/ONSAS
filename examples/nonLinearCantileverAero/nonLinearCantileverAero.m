...
%md Now the element aerodynmamic properties are defined. First the drag coefficient function located at the folder's example is declared into _userDragCoef_ field as:
...
numGaussPoints  = 4 ;
elements(2).elemTypeAero   = [0 d 0 numGaussPoints 4 ] ;
...
% %md Inside that function a ramp velocity profile $v_a(t) = 5*t*(t<10) + 5*t*(t>=10)$ is declared. This is an abrupt wind velocity load from 0 to $7$ m/s in $10$ s .
% %md
...
% %md
% %md ONSAS software is executed for the parameters above defined and the displacement solution of each load(time) step is saved into matUsLDDynamic matrix:
...
%md--------------------
%md A large dipslacements dynamic 3D case is presented as follows. This example is inspierd on [Vortex-Induced Vibration of Soft Corals](https://www.youtube.com/watch?v=Lf9Ffj5rGh8&ab_channel=FrederickGosselin).
%md---------------------
%md### MEBI parameters
%md---------------------
%md
...
%md
%md### analysisSettings  
%md Regarding the integration time method scheme, a classic $\alpha-HHT$ method is employed. This method is more numercially stable than Newmark, the keen reader is refereed to [this reference](https://onlinelibrary.wiley.com/doi/abs/10.1002/eqe.4290050306):
analysisSettings.methodName = 'alphaHHT';
analysisSettings.alphaHHT   =   -0.05   ;
%md the simulation time is defined such that:  
analysisSettings.deltaT     =  .1      ;
analysisSettings.finalTime  =  120       ;
%md The emulation of the vortex shedding vibration is generated by a synthetic wind veloctiy composed by two sinusoidal velocities. First a low frequency sinous is implemente along the mean flow direction $y$ and then a high frequency component  along $z$. The high frequency component is selected to produce resonance between the flow and the beam,  theafter the first mode frequency is calculated as follows:
...
%md The velocity function is assambled and plotted to ilustrate the reader
timeVecLD3d                 = linspace(0,analysisSettings.finalTime, analysisSettings.finalTime / analysisSettings.deltaT + 1) ;
windVelY = [] ; windVelZ = [] ;
...
%md
%md ONSAS software is executed for the parameters above defined and the displacement solution of each load(time) step is saved into matUsLD3D matrix:
[matUsLD3D, ~] = ONSAS( materials, elements, boundaryConds, initialConds, mesh, analysisSettings, otherParams ) ;
md 
%md
%md### Plots
%md
%md The wind velocty profile implemente plotted executing:  
fig3 = figure(3) ;
hold on,  grid on
% legend first point plot
plot(timeVecLD3d, windVelY(1),...
     'color', 'b', 'linewidth', lw, 'linestyle', '-','markersize', ms, 'marker', 'o')
plot(timeVecLD3d, windVelZ(1),...
     'color', 'r', 'linewidth', lw, 'linestyle', '-','markersize', ms, 'marker', 'o')
% markers plot
plot(timeVecLD3d(1:8:end), windVelY(1:8:end),...
     'color', 'b', 'linewidth', lw, 'linestyle', 'none','markersize', ms, 'marker', 'o')
plot(timeVecLD3d(1:8:end), windVelZ(1:8:end),...
     'color', 'r', 'linewidth', lw, 'linestyle', 'none','markersize', ms, 'marker', 'o')
% continium line plot
