...
[L, w, EI, rhoA, nuA, NR, cy_vec, uy_vec  ] = loadParamteters() ;
% according to the given parameters we obtain:
E = 10e6; Iyy = EI/E; t = (12*Iyy / w)^(1/3);
nu = 0.3 ; rho = 20000 ; G = E / (2 * (1+nu)) ;
% fluid properties
...
elements(2).elemCrossSecParams{2,1} = [ t w ] ;% number of Gauass integration points and elemTypeAero field:
...
elements(2).elemTypeAero   = [0 w 0 numGaussPoints computeAeroTangentMatrix ] ;
% The drag function name is:
elements(2).aeroCoefs = {'dragRectangle'; []; [] } ;
%
% boundaryConds
%----------------------------
...
%----------------------------
analysisSettings.fluidProps = {rhoA; nuA; 'windVelRecStatic'} ;
...
for windVelStep = 1:numLoadSteps - 1
    ...
    Cy(windVelStep) =  rhoA * normWindVel^2 * (L/2)^3 / (2 * EI)                              ;

    % numeric drag 
    FDragi = FDrag(windVelStep) ;
    R(windVelStep) =  abs(FDragi)/(1/2 * rhoA * normWindVel^2 * C_d * w * L/2 )               ;
...
analysisSettings.fluidProps = {rhoA; nuA; 'windVelRecDynamic'} ;
...
otherParams.problemName = 'dynamicReconfigurationRectangle' ;
%md
